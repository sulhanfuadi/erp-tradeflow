...
export function getNavigationItemsForRole(roleOrUser: RoleCarrier): NavigationItem[] {
  ...
  if (role === "inventory_manager") return all.filter((i) => ["/", "/products", "/orders", "/warehouses"].includes(i.path));
...
export function canAccessRoute(roleOrUser: RoleCarrier, pathname: string): boolean {
  ...
  if (pathname.startsWith("/procurement")) return role === "purchasing_manager" || role === "ar_analyst" || role === "ap_analyst";
