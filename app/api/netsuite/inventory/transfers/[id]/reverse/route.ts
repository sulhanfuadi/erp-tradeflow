...
export async function POST(
  ...
  try {
    ...
    const saved = await reverseStockTransfer(id, validation.data, guard.session!.id);
    ...
    logger.error("Error reversing inventory transfer:", error);
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to reverse inventory transfer",
      },
      { status: 400 },
    );
  }
