...
export default function ARAgingReport() {
  ...
  useEffect(() => {
    async function fetchReport() {
      try {
        const response = await fetch("/api/netsuite/reports/ar-aging");
        const json = await response.json();

        if (!response.ok) {
          throw new Error(json.error || "Failed to fetch AR aging report");
        }
  ...
  if (loading) return <div className="p-4 text-center">Loading A/R Aging...</div>;
  if (error) return <div className="p-4 text-center text-red-500">{error}</div>;
  if (!data.length) return <div className="p-4 text-center text-muted-foreground">No outstanding customer balances.</div>;

  return (
    <Card>
      <CardHeader>
        <CardTitle>A/R Aging Summary</CardTitle>
        <CardDescription>Customer balances by days overdue</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.map((row) => (
                <tr key={row.clientId} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
